...
public static class UILocale
{
    ...
    static readonly Dictionary<string, Pair> Table = new()
    {
        ...
        ["mode_btn_a"] = new("A  Класичний PID", "A  Classical PID"),
        ["mode_btn_b"] = new("B  Нечітка логіка", "B  Fuzzy logic"),
        ["mode_btn_c"] = new("C  Нейромережа", "C  Neural network"),
        ["mode_btn_d"] = new("D  Гібрид ★ рекомендовано", "D  Hybrid ★ recommended"),
        ...
        ["h_step1"] = new("КРОК 1 · ОБЕРІТЬ АЛГОРИТМ", "STEP 1 · SELECT ALGORITHM"),
        ["h_step2"] = new("КРОК 2 · КЕРУВАННЯ", "STEP 2 · CONTROL"),
        ["h_cam"] = new("КАМЕРА", "CAMERA"),
        ["h_export"] = new("ЕКСПОРТ ДОСЛІДЖЕННЯ", "RESEARCH EXPORT"),
        ["h_step3"] = new("КРОК 3 · УМОВИ ТЕСТУ", "STEP 3 · TEST CONDITIONS"),
        ["h_results"] = new("РЕЗУЛЬТАТИ ПОРІВНЯННЯ (% успіху)", "COMPARISON RESULTS (% success)"),
        ...
        ["btn_compare"] = new("⚡  ПОРІВНЯТИ ВСІ АЛГОРИТМИ", "⚡  COMPARE ALL ALGORITHMS"),
        ...
        // How-to
        ["how"] = new("① Алгоритм  →  ② Запуск  →  ③ Дивіться результат",
            "① Algorithm  →  ② Launch  →  ③ Watch result"),
        ["hint"] = new("① Справа: алгоритм (D)   ② «ЗАПУСТИТИ»   ③ Банер УСПІХ/НЕВДАЧА",
            "① Right: algorithm (D)   ② «START»   ③ SUCCESS/FAIL banner"),
        ["tip"] = new("Порада: D → ЗАПУСТИТИ. ЛКМ крутить камеру навколо ракети. Експорт → SimulationLogs/.",
            "Tip: D → START. LMB orbits camera around rocket. Export → SimulationLogs/."),
        ["bottom"] = new("Успіх: |Vy|<3.5 · нахил<7° · промах<25м · |Vh|<5  ·  ЛКМ orbit · колесо зум  ·  Експорт → SimulationLogs/",
            "Success: |Vy|<3.5 · tilt<7° · miss<25m · |Vh|<5  ·  LMB orbit · scroll zoom  ·  Export → SimulationLogs/"),
        ["graph_hint"] = new("min/max · ● поточне · жовта=0 · стрілки = зміна за крок",
            "min/max · ● current · yellow=0 · arrows = step change"),
        ...
        ["msg_started"] = new("▶ Посадка: {0}. ЛКМ — оберт навколо ракети.", "▶ Landing: {0}. LMB — orbit around rocket."),
        ...
        ["msg_folder"] = new("📂 {0}", "📂 {0}"),
        ["msg_cam_follow"] = new("🎯 Камера слідкує за ракетою. ЛКМ — оберт.", "🎯 Camera follows rocket. LMB — orbit."),
        ["msg_cam_manual"] = new("🖐 Ручне: ЛКМ/WASD — оберт навколо · колесо — зум.", "🖐 Manual: LMB/WASD — orbit · scroll — zoom."),
        ["msg_cam_traj"] = new("🛰 Повна траєкторія. ЛКМ — оберт · колесо — зум.", "🛰 Full trajectory. LMB — orbit · scroll — zoom."),
