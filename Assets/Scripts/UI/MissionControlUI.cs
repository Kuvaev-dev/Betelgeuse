...
public class MissionControlUI : MonoBehaviour
{
    ...
    // Keyboard UI navigation (Tab enters focus; arrows move; Esc clears)
    GameObject uiFocusGo;
    UnityEngine.UI.Outline uiFocusOutline;
    ...
    void ApplyThemeInPlace()
    {
        ...
        foreach (var img in canvasRoot.GetComponentsInChildren<Image>(true))
        {
            ...
            switch (n)
            {
                ...
                case "HelpHdrLine":
                case "HelpSecLine":
                case "GraphDetailHdrLine":
                    img.color = HeaderLineColor;
                    break;
                case "GraphDetailCard":
                    {
                        var gc = C_Panel; gc.a = 1f; img.color = gc;
                    }
                    break;
                case "GraphDetailFooter":
                    img.color = C_PanelSoft;
                    break;
                case "GraphDetailAccent":
                    img.color = new Color(C_Accent.r, C_Accent.g, C_Accent.b, 0.92f);
                    break;
                case "GraphDetailFrame":
                    {
        ...
        foreach (var btn in canvasRoot.GetComponentsInChildren<Button>(true))
        {
            ...
            // Scrim / click-swallow / graph hit-targets use None - do not chrome-paint them
            // (would overwrite modal card/scrim colors and recolor nested TMP labels).
            if (btn.transition == Selectable.Transition.None) continue;
        ...
        {
            if (t == null) return;
            var parentBtn = t.GetComponentInParent<Button>();
            // Real chrome buttons: PaintButton owns labels. Scrim/hit (None) may
            // parent body text (Help/GraphDetail hints) - still theme those here.
            if (parentBtn != null && parentBtn.transition != Selectable.Transition.None)
                return;
        ...
        T(txtGraphDetailTitle, C_Accent);
        T(txtGraphDetailHint, C_Secondary);
    ...
    void BuildTopChrome(Transform parent)
    {
        ...
        UpdatePauseButtonVisual();
        MenuBtn(row2.transform, (UILocale.T("top_demo") + "  M").ToUpperInvariant(), OnDefenseDemo, MenuBtnKind.Demo, chipW);
    ...
    /// <summary>
    /// True when EventSystem has an interactable Selectable selected - arrow keys navigate UI.
    /// WASD stay on camera; letter hotkeys stay global (unless typing in an InputField).
    /// </summary>
    ...
    void ClearUiFocusOutline()
    {
        if (uiFocusOutline != null)
        {
            Destroy(uiFocusOutline);
            uiFocusOutline = null;
        }
        uiFocusGo = null;
    }

    void UpdateUiFocusHighlight()
    {
        var es = UnityEngine.EventSystems.EventSystem.current;
        ...
        {
            var sel = go.GetComponent<Selectable>();
            if (sel == null || !sel.IsInteractable() || !go.activeInHierarchy)
                go = null;
        }
        if (go == uiFocusGo) return;
        ClearUiFocusOutline();
        uiFocusGo = go;
        if (go == null) return;

        // Accent ring on focused control (works even when Transition.None, e.g. graphs)
        var o = go.AddComponent<UnityEngine.UI.Outline>();
        Color ac = C_Accent;
        ac.a = 0.95f;
        o.effectColor = ac;
        o.effectDistance = new Vector2(2.4f, -2.4f);
        o.useGraphicAlpha = true;
        uiFocusOutline = o;
    }

    void ConfigureUiNavigation()
    {
        if (canvasRoot == null) return;
        var selectables = canvasRoot.GetComponentsInChildren<Selectable>(true);
        for (int i = 0; i < selectables.Length; i++)
        {
            var s = selectables[i];
            if (s == null) continue;

            // Overlays / graph tiles / scrollbars: mouse only — not Tab/arrow targets
            string n = s.gameObject.name;
            if (n == "HelpOverlay" || n == "GraphDetailOverlay"
                || n.StartsWith("GraphRoot_", System.StringComparison.Ordinal)
                || n.IndexOf("Scrollbar", System.StringComparison.OrdinalIgnoreCase) >= 0)
            {
    ...
    static bool IsUiNavTarget(Selectable s)
    {
        ...
        if (n == "HelpOverlay" || n == "GraphDetailOverlay")
            return false;
        // Skip window-caption chrome (min/max/close) - not part of Mission Control workflow
        if (n.StartsWith("Cap", System.StringComparison.Ordinal))
            return false;
        // Graph tiles stay mouse-clickable; keep them out of Tab/arrow order
        if (n.StartsWith("GraphRoot_", System.StringComparison.Ordinal))
            return false;
        if (n.IndexOf("Scrollbar", System.StringComparison.OrdinalIgnoreCase) >= 0)
            return false;
        return true;
    }

    List<Selectable> CollectFocusableSelectables()
    {
        var list = new List<Selectable>(64);
        var seen = new HashSet<int>();

        void AddFrom(GameObject root, bool includeInactive)
        {
            if (root == null) return;
            var arr = root.GetComponentsInChildren<Selectable>(includeInactive);
            for (int i = 0; i < arr.Length; i++)
            {
                var s = arr[i];
                if (!IsUiNavTarget(s)) continue;
        ...
        AddFrom(canvasRoot, false);
        // Conditions are sometimes detached/reparented - always merge them when active
        if (conditionSectionGo != null && conditionSectionGo.activeInHierarchy)
            AddFrom(conditionSectionGo, false);
    ...
    void NavigateUiFocus(bool reverse)
    {
        var all = CollectFocusableSelectables();
        ...
        if (cur != null)
        {
            ...
            {
                if (all[i] != null && all[i].gameObject == cur)
                {
                    idx = i;
                    break;
                }
        ...
        if (idx < 0)
            idx = reverse ? all.Count - 1 : 0;
        else
            idx = reverse ? (idx - 1 + all.Count) % all.Count : (idx + 1) % all.Count;

        es.SetSelectedGameObject(all[idx].gameObject);
        EnsureUiFocusVisible(all[idx]);
    }

    void NavigateUiFocusDirectional(UnityEngine.EventSystems.MoveDirection dir)
    {
        var all = CollectFocusableSelectables();
        if (all.Count == 0) return;
        var es = UnityEngine.EventSystems.EventSystem.current;
        ...
        if (idx < 0)
        {
            es.SetSelectedGameObject(all[0].gameObject);
            EnsureUiFocusVisible(all[0]);
            return;
        }

        // Sliders: Left/Right nudge value; Up/Down move focus
        var curSel = all[idx];
        if (curSel is Slider slider
            && (dir == UnityEngine.EventSystems.MoveDirection.Left
                || dir == UnityEngine.EventSystems.MoveDirection.Right))
        {
            float step = slider.wholeNumbers ? 1f : Mathf.Max(0.01f, (slider.maxValue - slider.minValue) * 0.02f);
            slider.value = Mathf.Clamp(
                slider.value + (dir == UnityEngine.EventSystems.MoveDirection.Left ? -step : step),
                slider.minValue, slider.maxValue);
            return;
        }

        Vector3 origin = curSel.transform.position;
        Selectable best = null;
        float bestScore = float.MaxValue;
        for (int i = 0; i < all.Count; i++)
        {
            if (i == idx || all[i] == null) continue;
            Vector3 d = all[i].transform.position - origin;
            bool ok = false;
            float primary = 0f, lateral = 0f;
            switch (dir)
            {
                case UnityEngine.EventSystems.MoveDirection.Up:
                    ok = d.y > 2f; primary = d.y; lateral = Mathf.Abs(d.x); break;
                case UnityEngine.EventSystems.MoveDirection.Down:
                    ok = d.y < -2f; primary = -d.y; lateral = Mathf.Abs(d.x); break;
                case UnityEngine.EventSystems.MoveDirection.Left:
                    ok = d.x < -2f; primary = -d.x; lateral = Mathf.Abs(d.y); break;
                case UnityEngine.EventSystems.MoveDirection.Right:
                    ok = d.x > 2f; primary = d.x; lateral = Mathf.Abs(d.y); break;
            }
            if (!ok) continue;
            float score = primary + lateral * 2.5f;
            if (score < bestScore)
            {
                bestScore = score;
                best = all[i];
        ...
        if (best == null)
        {
            int next = dir == UnityEngine.EventSystems.MoveDirection.Up
                       || dir == UnityEngine.EventSystems.MoveDirection.Left
        ...
        EnsureUiFocusVisible(best);
    }

    void EnsureUiFocusVisible(Selectable sel)
    {
        if (sel == null) return;
        ...
        scroll.viewport.GetWorldCorners(vCorners);
        float itemTop = cCorners[1].y;
        float itemBot = cCorners[0].y;
        float viewTop = vCorners[1].y;
        float viewBot = vCorners[0].y;
        float dy = 0f;
        if (itemTop > viewTop) dy = itemTop - viewTop;
        else if (itemBot < viewBot) dy = itemBot - viewBot;
        if (Mathf.Abs(dy) < 0.5f) return;

        var contentRt = scroll.content;
        Vector3 local = contentRt.InverseTransformVector(new Vector3(0f, dy, 0f));
        var ap = contentRt.anchoredPosition;
        ap.y -= local.y;
        contentRt.anchoredPosition = ap;
    }

    void ActivateFocusedUiControl()
    {
        var es = UnityEngine.EventSystems.EventSystem.current;
        if (es == null) return;
        var go = es.currentSelectedGameObject;
        ...
        // Enter activates focused control. Never treat this as the M/Demo path.
        ...
        var ped = new UnityEngine.EventSystems.BaseEventData(es);
        UnityEngine.EventSystems.ExecuteEvents.Execute(
            go, ped, UnityEngine.EventSystems.ExecuteEvents.submitHandler);
    }

    void HandleHotkeys()
    {
        var es = UnityEngine.EventSystems.EventSystem.current;
        GameObject selGo = es != null ? es.currentSelectedGameObject : null;
        bool typing = false;
        bool uiFocus = false;
        if (selGo != null)
        {
            if (selGo.GetComponent<TMPro.TMP_InputField>() != null
                || selGo.GetComponent<UnityEngine.UI.InputField>() != null)
                typing = true;
            var sel = selGo.GetComponent<Selectable>();
            uiFocus = sel != null && sel.IsInteractable();
        }

        // Click on scene (not UI) clears keyboard focus so camera arrows resume
        if (uiFocus && (Input.GetMouseButtonDown(0) || Input.GetMouseButtonDown(1)))
        {
        ...
        // Tab / Shift+Tab — enter or cycle Mission Control UI focus (also leaves InputField)
        if (Input.GetKeyDown(KeyCode.Tab))
        {
            if (typing)
            {
                var tmp = selGo != null ? selGo.GetComponent<TMP_InputField>() : null;
                if (tmp != null) tmp.DeactivateInputField();
                var uif = selGo != null ? selGo.GetComponent<UnityEngine.UI.InputField>() : null;
                if (uif != null) uif.DeactivateInputField();
            }
            bool reverse = Input.GetKey(KeyCode.LeftShift) || Input.GetKey(KeyCode.RightShift);
            NavigateUiFocus(reverse);
            var focused = es != null ? es.currentSelectedGameObject : null;
            if (focused != null)
                EnsureUiFocusVisible(focused.GetComponent<Selectable>());
            return;
        }

        // While typing in a number field, only Esc clears — do not steal digits/letters
        if (typing)
        {
        ...
        // Arrows only for UI navigation when focus is active (never WASD)
        if (uiFocus)
        {
            if (Input.GetKeyDown(KeyCode.LeftArrow))
            {
                NavigateUiFocusDirectional(UnityEngine.EventSystems.MoveDirection.Left);
                return;
            }
            if (Input.GetKeyDown(KeyCode.RightArrow))
            {
                NavigateUiFocusDirectional(UnityEngine.EventSystems.MoveDirection.Right);
                return;
            }
            if (Input.GetKeyDown(KeyCode.UpArrow))
            {
                NavigateUiFocusDirectional(UnityEngine.EventSystems.MoveDirection.Up);
                return;
            }
            if (Input.GetKeyDown(KeyCode.DownArrow))
            {
                NavigateUiFocusDirectional(UnityEngine.EventSystems.MoveDirection.Down);
                return;
            }
        }

        // Enter activates focused control (Submit). Space is always Start when no modal.
        ...
        bool modal = helpVisible || graphDetailVisible;

        if (Input.GetKeyDown(KeyCode.H) && !ctrl) TogglePanels();
        // Space always starts (unless modal/typing). Enter — not Space — activates focused widgets.
        // Focused Start must not bind M; M remains Demo hotkey below.
        if (Input.GetKeyDown(KeyCode.Space) && !modal) OnStartLanding();
        ...
        if (Input.GetKeyDown(KeyCode.Escape))
        {
            ...
            if (resultShown) { HideLandingResult(); return; }
            if (uiFocus) { ClearUiFocus(); return; }
            OnStop();
            return;
        }
        if (Input.GetKeyDown(KeyCode.F1) || Input.GetKeyDown(KeyCode.Slash) || Input.GetKeyDown(KeyCode.Question))
            ToggleHelp();
        if (Input.GetKeyDown(KeyCode.M) && !ctrl)
            OnDefenseDemo();
    ...
    void BuildHelpOverlay(Transform parent)
    {
        ...
        KeyRow("Tab · Arrows · Enter", "help_k_nav");
    ...
    void BuildGraphDetailOverlay(Transform parent)
    {
        ...
        crt.sizeDelta = new Vector2(cardW, cardH);
        Outline(card, 1.5f);
        // Card Image.raycastTarget blocks scrim (same as HelpCard). No swallow Button -
        // PaintButton would otherwise treat the card as chrome and recolor nested labels.
    ...
    static void EnsureEventSystem()
    {
        ...
        es.enabled = true;
        es.sendNavigationEvents = false; // arrows/Enter/Space owned by HandleHotkeys
    ...
    static void ApplyBtnColorBlock(ref ColorBlock colors)
    {
        ...
        if (UiTheme.IsLightBackground)
        {
            ...
            // Keyboard focus: cool lift so selected stands out from hover
            colors.selectedColor = new Color(0.90f, 0.95f, 1.12f, 1f);
        }
        else
        {
            colors.highlightedColor = new Color(1.1f, 1.1f, 1.12f, 1f);
            colors.pressedColor = new Color(0.85f, 0.85f, 0.88f, 1f);
            colors.selectedColor = new Color(1.22f, 1.26f, 1.38f, 1f);
        }
    }
    ...
    void PlaceConditionSection(Transform root, ref float y, float pad, float inner, float gap)
    {
        ...
        // Make noise/wind/defense toggles+sliders Tab/arrow reachable
        foreach (var s in conditionSectionGo.GetComponentsInChildren<Selectable>(true))
        {
            if (s == null) continue;
            var nav = s.navigation;
            nav.mode = Navigation.Mode.Automatic;
        ...
        string labelKey = null, string unitKey = null)
    {
        ...
        sld.navigation = new Navigation { mode = Navigation.Mode.Automatic };
        ...
        string labelKey = null)
    {
        ...
        var tc = ColorBlock.defaultColorBlock;
        ApplyBtnColorBlock(ref tc);
        toggle.colors = tc;
