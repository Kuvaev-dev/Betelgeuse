...
public class CameraFollow : MonoBehaviour
{
    ...
    bool IsOrbitKeyHeld()
    {
        // WASD/QE/zoom always orbit. Arrow keys yield to Mission Control UI focus only.
        bool wasd = Input.GetKey(KeyCode.A) || Input.GetKey(KeyCode.D)
        ...
        if (MissionControlUI.HasKeyboardFocus())
            return wasd;
        return wasd
    ...
    void HandleInput()
    {
        ...
        // WASD/QE/zoom always for camera. Arrows orbit only when UI focus is clear.
        {
            float k = keyOrbitSpeed * Time.unscaledDeltaTime;
            float yD = 0f, pD = 0f, zD = 0f;
            bool uiNav = MissionControlUI.HasKeyboardFocus();
            if (Input.GetKey(KeyCode.A) || Input.GetKey(KeyCode.Q)) yD -= k;
            if (Input.GetKey(KeyCode.D) || Input.GetKey(KeyCode.E)) yD += k;
            if (Input.GetKey(KeyCode.W)) pD += k * 0.7f;
            if (Input.GetKey(KeyCode.S)) pD -= k * 0.7f;
            if (!uiNav)
            {
                if (Input.GetKey(KeyCode.LeftArrow)) yD -= k;
                if (Input.GetKey(KeyCode.RightArrow)) yD += k;
                if (Input.GetKey(KeyCode.UpArrow)) pD += k * 0.7f;
                if (Input.GetKey(KeyCode.DownArrow)) pD -= k * 0.7f;
            }
